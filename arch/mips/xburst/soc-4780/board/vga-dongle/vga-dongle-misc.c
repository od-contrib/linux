...
static struct jz_battery_platform_data vga_dongle_battery_pdata = {
	.info = {
		.max_vol        = 4050,
		.min_vol        = 3600,
		.usb_max_vol    = 4100,
...
/* mmc */
	...
	jz_device_register(&jz_msc0_device, &vga_dongle_tf_pdata);
#endif
#ifdef CONFIG_MMC1_JZ4780
	jz_device_register(&jz_msc1_device, &vga_dongle_sdio_pdata);
#endif
#ifdef CONFIG_MMC2_JZ4780
	jz_device_register(&jz_msc2_device, &vga_dongle_tf_pdata);
#endif
/* sound */
...
/* panel and bl */
	...
	platform_device_register(&vga_dongle_backlight_device);
#endif
...
/* ADC*/
#ifdef CONFIG_BATTERY_JZ4780
	jz_device_register(&jz_adc_device, &vga_dongle_battery_pdata);
#endif
/* uart */
...
arch_initcall(vga_dongle_board_init);
