 ...
 *
 * when usb_gadget_probe_driver() called, there's no place to get the struct dwc2,
 * so we need to maintain a global one here!
 */
...
int dwc2_gadget_init(struct dwc2 *dwc)
{
	...
	dwc->gadget.dev.release	  = dwc2_gadget_release;
...
void dwc2_gadget_plug_change(int plugin) {
	...
	if (plugin && (dwc2_get_id_level(dwc) == 0)) {
		extern_vbus_mode = 1;
...
/**
 * "Callback" function for usb_composite_probe
 *
			...
			int (*bind)(struct usb_gadget *))
{
	...
	if (dwc == NULL) {
		pr_err("%s: controller not ready\n", __func__);
		return -ENODEV;
	}

	dev_info(dwc->dev, "registering gadget driver '%s'\n",
		driver->driver.name);
	...
	if (driver->speed == USB_SPEED_UNKNOWN ||
		...
		dev_err(dwc->dev, "%s: gadget driver callbacks missing!\n", __func__);
		return -EINVAL;
	}

	if (dwc->gadget_driver != NULL) {
		dev_err(dwc->dev, "%s: controller busy on %s!\n",
			__func__, dwc->gadget_driver->driver.name);
	}

	dwc->gadget.dev.driver = &driver->driver;

	dev_info(dwc->dev, "bind to driver %s\n", driver->driver.name);
	ret = bind(&dwc->gadget);
	if (ret) {
		dev_err(dwc->dev, "bind to driver %s failed: ret = %d\n",
			driver->driver.name, ret);
		dwc->gadget_driver = NULL;
		dwc->gadget.dev.driver = 0;
		return ret;
	}
...
int usb_gadget_unregister_driver(struct usb_gadget_driver *driver)
{
	...
	dev_info(dwc->dev, "unregister gadget driver %s\n",
		driver->driver.name);

	if (driver != dwc->gadget_driver) {
		dev_err(dwc->dev, "%s driver not match, current driver is %s\n",
