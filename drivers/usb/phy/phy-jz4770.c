...
static int jz4770_phy_init(struct usb_phy *phy)
{
	...
	if (err) {
		dev_err(priv->dev, "Unable to enable VCC");
		return err;
	}
...
static enum usb_charger_type jz4770_phy_charger_detect(struct usb_phy *usb_phy)
{
	return SDP_TYPE;
}
...
static int jz4770_phy_probe(struct platform_device *pdev)
{
	...
	priv->phy.charger_detect = jz4770_phy_charger_detect;
