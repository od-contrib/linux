...
static int jz4770_phy_set_vbus(struct usb_otg *otg, bool enable)
{
...
static int jz4770_phy_init(struct usb_phy *phy)
{
	...
	jz4770_phy_set_peripheral(&priv->otg, NULL);

	return 0;
}

static int jz4770_vbus_changed(struct notifier_block *nb,
			       unsigned long state, void *data)
{
	struct jz4770_phy *priv = container_of(nb, struct jz4770_phy,
					       phy.vbus_nb);

	dev_notice(priv->dev, "VBUS changed to state %lu", state);

	return musb_mailbox(state ? MUSB_VBUS_VALID : MUSB_VBUS_OFF);
}

static int jz4770_id_changed(struct notifier_block *nb,
			     unsigned long state, void *data)
{
	struct jz4770_phy *priv = container_of(nb, struct jz4770_phy,
					       phy.id_nb);

	dev_notice(priv->dev, "ID changed to state %lu", state);

	return musb_mailbox(state ? MUSB_ID_GROUND : MUSB_ID_FLOAT);
}
...
static int jz4770_phy_probe(struct platform_device *pdev)
{
	...
	priv->phy.vbus_nb.notifier_call = jz4770_vbus_changed;
	priv->phy.id_nb.notifier_call = jz4770_id_changed;
	...
	priv->otg.set_vbus = jz4770_phy_set_vbus;
	...
	priv->clk = devm_clk_get(dev, "otg_phy");
	if (IS_ERR(priv->clk)) {
	...
	priv->vbus_supply = devm_regulator_get(dev, "vbus");
