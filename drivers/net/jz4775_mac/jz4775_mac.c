...
#define JZMAC_DRV_DESC		"JZ4775 on-chip Ethernet MAC driver"
...
__attribute__((__unused__)) static void jzmac_dump_rx_desc(struct jz4775_mac_local *lp) {
	int i = 0;
	printk("\n===================rx====================\n");
...
__attribute__((__unused__)) static void jzmac_dump_tx_desc(struct jz4775_mac_local *lp) {
	int i = 0;
	printk("\n===================tx====================\n");
	printk("count = %d, next_to_use = %d next_to_clean = %d\n",
...
__attribute__((__unused__)) static void jzmac_dump_all_desc(struct jz4775_mac_local *lp) {
	jzmac_dump_rx_desc(lp);
	jzmac_dump_tx_desc(lp);
}

__attribute__((__unused__)) static int get_rx_index_by_desc(struct jz4775_mac_local *lp, DmaDesc *desc) {
	int i = 0;
...
__attribute__((__unused__)) static void jzmac_phy_dump(struct jz4775_mac_local *lp) {
...
static void jzmac_restart_rx_dma(struct jz4775_mac_local *lp) {
	synopGMAC_enable_dma_rx(gmacdev);
}

static void jzmac_alloc_rx_buffers(struct jz4775_mac_local *lp, int cleaned_count,
				   int restart_dma) {
	int i = 0;
...
static int desc_list_init_rx(struct jz4775_mac_local *lp) {
	int i;
	int size;
...
static void desc_list_free_rx(struct jz4775_mac_local *lp) {
...
static void jzmac_take_desc_ownership_rx(struct jz4775_mac_local *lp) {
	int i = 0;
...
static void desc_list_reinit_rx(struct jz4775_mac_local *lp) {
	DmaDesc *desc;
	int i = 0;
...
static int desc_list_init_tx(struct jz4775_mac_local *lp) {
	int i;
	int size;
...
__attribute__((__unused__)) static int get_tx_index_by_desc(struct jz4775_mac_local *lp, DmaDesc *desc) {
	int i = 0;
...
static void jzmac_unmap_and_free_tx_resource(struct jz4775_mac_local *lp,
					     struct jzmac_buffer *buffer_info)
{
...
static void desc_list_free_tx(struct jz4775_mac_local *lp) {
	struct jzmac_buffer *b;
	int i = 0;
...
static void jzmac_take_desc_ownership_tx(struct jz4775_mac_local *lp) {
	int i = 0;
...
static void desc_list_reinit_tx(struct jz4775_mac_local *lp) {
	int i = 0;
	DmaDesc *desc;
	struct jzmac_buffer *b;
...
static void desc_list_free(struct jz4775_mac_local *lp)
{
	desc_list_free_rx(lp);
	desc_list_free_tx(lp);
}

static void desc_list_reinit(struct jz4775_mac_local *lp) {
	desc_list_reinit_rx(lp);
	desc_list_reinit_tx(lp);
	//jzmac_dump_all_desc(lp);
}

static int desc_list_init(struct jz4775_mac_local *lp)
{
...
static void jz4775_mac_adjust_link(struct net_device *dev)
{
...
static int mii_probe(struct net_device *dev)
{
	...
	phydev = phy_connect(dev, dev_name(&phydev->dev), &jz4775_mac_adjust_link,
...
void jzmac_update_stats(struct jz4775_mac_local *lp)
{
...
static void jzmac_watchdog(unsigned long data) {
	struct jz4775_mac_local *lp = (struct jz4775_mac_local *)data;
...
static void jzmac_tx_queue(struct jz4775_mac_local *lp,
			   struct jzmac_tx_ring *tx_ring)
{
...
static int jz4775_mac_hard_start_xmit(struct sk_buff *skb,
...
static bool jzmac_clean_tx_irq(struct jz4775_mac_local *lp) {
	struct net_device *netdev = lp->netdev;
	DmaDesc *desc;
	struct jzmac_buffer *buffer_info;
	struct jzmac_tx_ring *tx_ring = &lp->tx_ring;
...
static bool jzmac_clean_rx_irq(struct jz4775_mac_local *lp,
...
static int jzmac_clean(struct napi_struct *napi, int budget) {
	struct jz4775_mac_local *lp = container_of(napi, struct jz4775_mac_local, napi);
	int tx_cleaned = 0;
	int work_done = 0;
...
static irqreturn_t jz4775_mac_interrupt(int irq, void *data)
{
...
static void jz4775_mac_stop_activity(struct jz4775_mac_local *lp) {
	synopGMAC_disable_interrupt_all(gmacdev);
...
static void jz4775_mac_disable(struct jz4775_mac_local *lp) {
...
static void jz4775_mac_configure(struct jz4775_mac_local *lp) {
#ifdef ENH_DESC_8W
...
/*
 * Enable Interrupts, Receive, and Transmit(The same sequence as jz4775_mac_open, only a bit different)
 */
static void jz4775_mac_enable(struct jz4775_mac_local *lp) {
	jz4775_mac_configure(lp);
...
static void jzmac_reinit_locked(struct jz4775_mac_local *lp)
{
	WARN_ON(in_interrupt());
	jz4775_mac_disable(lp);
	jz4775_mac_enable(lp);
}

/* Our watchdog timed out. Called by the networking layer */
static void jz4775_mac_tx_timeout(struct net_device *dev)
{
	struct jz4775_mac_local *lp = netdev_priv(dev);
...
{
	struct jz4775_mac_local *lp =
		container_of(work, struct jz4775_mac_local, reset_task);
...
static int jz4775_mac_set_mac_address(struct net_device *dev, void *p) {
 ...
 */
static int jz4775_mac_open(struct net_device *dev)
{
	struct jz4775_mac_local *lp = netdev_priv(dev);
	int retval;
 ...
 */
static int jz4775_mac_close(struct net_device *dev)
{
	struct jz4775_mac_local *lp = netdev_priv(dev);
...
static struct net_device_stats *jz4775_mac_get_stats(struct net_device *netdev)
{
	struct jz4775_mac_local *lp = netdev_priv(netdev);
...
static int jz4775_mac_change_mtu(struct net_device *netdev, int new_mtu) {
	printk("===>new_mtu = %d\n", new_mtu);
#if 0
...
static const struct net_device_ops jz4775_mac_netdev_ops = {
	.ndo_open		= jz4775_mac_open,
	.ndo_stop		= jz4775_mac_close,
	.ndo_start_xmit		= jz4775_mac_hard_start_xmit,
	.ndo_get_stats		= jz4775_mac_get_stats,
	.ndo_set_mac_address	= jz4775_mac_set_mac_address,
	.ndo_tx_timeout		= jz4775_mac_tx_timeout,
	.ndo_set_multicast_list	= jzmac_set_multicast_list,
	.ndo_validate_addr	= eth_validate_addr,
	.ndo_change_mtu		= jz4775_mac_change_mtu,
	.ndo_do_ioctl		= jzmac_do_ioctl,
};

static int __devinit jz4775_mac_probe(struct platform_device *pdev)
{
	...
	ndev = alloc_etherdev(sizeof(struct jz4775_mac_local));
	if (!ndev) {
	...
	if (!pdev->dev.platform_data) {
		dev_err(&pdev->dev, "Cannot get platform device jz4775_mii_bus!\n");
		rc = -ENODEV;
	...
	rc = request_irq(IRQ_ETH, jz4775_mac_interrupt,
			 IRQF_DISABLED, "jzmac", ndev);
	if (rc) {
		dev_err(&pdev->dev, "Cannot request JZ4775 MAC IRQ!\n");
		rc = -EBUSY;
...
static int __devexit jz4775_mac_remove(struct platform_device *pdev)
{
	struct net_device *ndev = platform_get_drvdata(pdev);
	struct jz4775_mac_local *lp = netdev_priv(ndev);
...
#ifdef CONFIG_PM
static int jz4775_mac_suspend(struct platform_device *pdev, pm_message_t mesg)
{
	struct net_device *net_dev = platform_get_drvdata(pdev);

	if (netif_running(net_dev))
		jz4775_mac_close(net_dev);

	return 0;
}

static int jz4775_mac_resume(struct platform_device *pdev)
{
	struct net_device *net_dev = platform_get_drvdata(pdev);

	if (netif_running(net_dev))
		jz4775_mac_open(net_dev);

	return 0;
}
#else
#define jz4775_mac_suspend NULL
#define jz4775_mac_resume NULL
#endif	/* CONFIG_PM */
...
static int __devinit jz4775_mii_bus_probe(struct platform_device *pdev)
{
	...
	if (miibus == NULL)
		goto out_err_alloc;
	miibus->read = jz4775_mdiobus_read;
	miibus->write = jz4775_mdiobus_write;
	miibus->reset = jz4775_mdiobus_reset;

	miibus->parent = &pdev->dev;
	miibus->name = "jz4775_mii_bus";
...
static int __devexit jz4775_mii_bus_remove(struct platform_device *pdev)
{
...
static struct platform_driver jz4775_mii_bus_driver = {
	.probe = jz4775_mii_bus_probe,
	.remove = __devexit_p(jz4775_mii_bus_remove),
	.driver = {
		.name = "jz4775_mii_bus",
		.owner	= THIS_MODULE,
	},
...
static struct platform_driver jz4775_mac_driver = {
	.probe = jz4775_mac_probe,
	.remove = __devexit_p(jz4775_mac_remove),
	.resume = jz4775_mac_resume,
	.suspend = jz4775_mac_suspend,
	.driver = {
		.name = JZMAC_DRV_NAME,
		.owner	= THIS_MODULE,
	},
...
static int __init jz4775_mac_init(void)
{
#ifndef CONFIG_MDIO_GPIO
	int ret;
#endif
...
#ifndef CONFIG_MDIO_GPIO
	ret = platform_driver_register(&jz4775_mii_bus_driver);
	if (!ret) {
		return platform_driver_register(&jz4775_mac_driver);
	}
	return -ENODEV;
#else
	return platform_driver_register(&jz4775_mac_driver);
...
module_init(jz4775_mac_init);

static void __exit jz4775_mac_cleanup(void)
{
	platform_driver_unregister(&jz4775_mac_driver);
#ifndef CONFIG_MDIO_GPIO
	platform_driver_unregister(&jz4775_mii_bus_driver);
#endif
}

module_exit(jz4775_mac_cleanup);
