 ...
 * Copyright (c) 2017-2020 Artur Rojek <contact@artur-rojek.eu>
 * Copyright (c) 2017-2020 Paul Cercueil <paul@crapouillou.net>
 */
...
static bool spi_ingenic_can_dma(struct spi_controller *ctlr,
				...
				struct spi_transfer *xfer)
{
	/* DMA should always be available. */
	return 1;
}
...
static int spi_ingenic_probe(struct platform_device *pdev)
{
	...
	ctlr = spi_alloc_master(dev, sizeof(*priv));
	if (!ctlr) {
	...
	if (IS_ERR(priv->clk)) {
		ret = dev_err_probe(dev, PTR_ERR(priv->clk),
				    "Unable to get clock.\n");
		goto out_free;
	}

	priv->mem_res = platform_get_resource(pdev, IORESOURCE_MEM, 0);
	base = devm_ioremap_resource(dev, priv->mem_res);
	if (IS_ERR(base)) {
		ret = PTR_ERR(base);
		goto out_free;
	}
