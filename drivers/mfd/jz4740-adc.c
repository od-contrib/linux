...
static void jz4740_adc_irq_demux(struct irq_desc *desc)
{
	...
	status = readb(adc->gc->reg_base + JZ_REG_ADC_STATUS);

	for (i = 0; i < JZ_ADC_IRQ_NUM; ++i) {
		if (status & BIT(i))
			generic_handle_irq(irq_linear_revmap(adc->irq_domain, i));
	}
...
static void jz4740_adc_destroy_irq_chip(struct irq_chip_generic *gc)
{
	irq_destroy_generic_chip(gc, IRQ_MSK(JZ_ADC_IRQ_NUM),
...
static int jz4740_adc_probe(struct platform_device *pdev)
{
	...
	if (ret)
		return ret;
	adc->gc = gc = irq_get_domain_generic_chip(adc->irq_domain, 0);
