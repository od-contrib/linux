/*
 * Real Time Clock interface for Jz4780.
 *
...
static int jz4780_rtc_read_time(struct device *dev, struct rtc_time *tm)
{
	unsigned int tmp;
	struct jz_rtc *rtc = dev_get_drvdata(dev);
	...
	if (rtc_valid_tm(tm) < 0) {
		...
		jz4780_rtc_set_time(dev, &default_tm);
		tmp = jzrtc_readl(rtc, RTC_RTCSR);
		rtc_time_to_tm(tmp, tm);
	}
	return 0;
...
static int jz4780_rtc_read_alarm(struct device *dev, struct rtc_wkalrm *alrm)
{
...
static int jz4780_rtc_set_alarm(struct device *dev, struct rtc_wkalrm *alrm)
{
	int ret = 0;
	unsigned long time;
	unsigned int tmp;
...
static void jz4780_rtc_enable(struct jz_rtc *rtc)
{
...
static int jz4780_rtc_probe(struct platform_device *pdev)
{
	struct jz_rtc *rtc;
	int ret;
	...
	pr_debug("jz4780_rtc: tick irq %d\n",  rtc->irq);
	...
	rtc->rtc = rtc_device_register(pdev->name, &pdev->dev, &jz4780_rtc_ops,
			THIS_MODULE);
...
static int jz4780_rtc_remove(struct platform_device *pdev)
{
...
static struct platform_driver jz4780_rtc_driver = {
	.probe		= jz4780_rtc_probe,
	.remove		= jz4780_rtc_remove,
...
static int __init jz4780_rtc_init(void)
{
	return platform_driver_register(&jz4780_rtc_driver);
}

static void __exit jz4780_rtc_exit(void)
{
	platform_driver_unregister(&jz4780_rtc_driver);
}

module_init(jz4780_rtc_init);
module_exit(jz4780_rtc_exit);

MODULE_AUTHOR("cwwang <cwwang@ingenic.cn>");
MODULE_DESCRIPTION("JZ4780 Realtime Clock Driver (RTC)");
MODULE_LICENSE("GPL");
MODULE_ALIAS("platform:jz4780-rtc");
