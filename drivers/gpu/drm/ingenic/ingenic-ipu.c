					    ...
					    struct drm_plane_state *oldstate)
{
	...
	ipu_state = ingenic_ipu_get_new_priv_state(ipu, oldstate->state);
	...
	drm_gem_cma_sync_data(ipu->drm, oldstate, state);
					  ...
					  struct drm_plane_state *state)
{
	...
	ipu_state = ingenic_ipu_get_priv_state(ipu, state->state);
	if (IS_ERR(ipu_state))
