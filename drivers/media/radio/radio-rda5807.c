					      ...
					      const struct i2c_device_id *id)
{
	...
	device_property_read_u16(dev, "rdamicro,lna-current", &lna_current);
	for (i = 0; i < ARRAY_SIZE(rda5807_lna_current); i++)
		if (rda5807_lna_current[i] == lna_current)
			radio->input_flags = i;

	if (device_property_read_bool(dev, "rdamicro,lnan"))
		radio->input_flags |= RDA5807_LNA_PORT_N;
	if (device_property_read_bool(dev, "rdamicro,lnap"))
		radio->input_flags |= RDA5807_LNA_PORT_P;

	if (device_property_read_bool(dev, "rdamicro,i2s-out"))
		radio->output_flags |= RDA5807_OUTPUT_AUDIO_I2S;
	if (device_property_read_bool(dev, "rdamicro,analog-out"))
