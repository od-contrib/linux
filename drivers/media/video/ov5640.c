		...
		const char *buf ,int count)
{
...
#if 1
	{
		...
		if (cam_t_i == 10) {
			...
			cam_t_i = 11;
			while(--cam_t_j)
				printk("cam%d : i2c2_time 0  = %lld, i2c2_time 1"
						"= %lld, time = %lld\n",
						cam_t_j,
						cam_t0_buf[cam_t_j],
						cam_t1_buf[cam_t_j],
			      ...
			      struct i2c_client *client)
{
	 ...
	 */
#if 0
	retval = i2c_smbus_write_byte_data(client, 0x3008, 0x80);
#endif
...
#if 0
	retval_high = i2c_smbus_read_byte_data(client, REG_CHIP_ID_HIGH);
#endif
...
#if 0
	retval_low = i2c_smbus_read_byte_data(client, REG_CHIP_ID_LOW);
#endif
			...
			const struct i2c_device_id *did)
{
	...
	if(!strcmp(client->name, "ov5640-back")) {
		client->addr -= 1;
	}

	if (!icd) {
		dev_err(&adapter->dev, "OV5640: missing soc-camera data!\n");
		return -EINVAL;
	}
	...
	if (!icl) {
		dev_err(&adapter->dev,
			"OV5640: Missing platform_data for driver\n");
		return -EINVAL;
	}
