...
static void send_panel_command(struct jzfb *jzfb, u32 cmd) {
	u16 slcd_cfg = jz_read(jzfb, JZ_REG_SLCD_CFG);
	switch (slcd_cfg & SLCD_CFG_CWIDTH_MASK) {
	case SLCD_CFG_CWIDTH_8BIT:
		while (jz_read(jzfb, JZ_REG_SLCD_STATE) & SLCD_STATE_BUSY);
		jz_write(jzfb, SLCD_DATA_RS_COMMAND | ((cmd&0xff00) >> 8), JZ_REG_SLCD_DATA);
		while (jz_read(jzfb, JZ_REG_SLCD_STATE) & SLCD_STATE_BUSY);
		jz_write(jzfb, SLCD_DATA_RS_COMMAND | ((cmd&0xff) >> 0), JZ_REG_SLCD_DATA);
		break;
	case SLCD_CFG_CWIDTH_16BIT:
		while (jz_read(jzfb, JZ_REG_SLCD_STATE) & SLCD_STATE_BUSY);
		jz_write(jzfb, SLCD_DATA_RS_COMMAND | (cmd&0xffff), JZ_REG_SLCD_DATA);
		break;
	case SLCD_CFG_CWIDTH_18BIT:
		while (jz_read(jzfb, JZ_REG_SLCD_STATE) & SLCD_STATE_BUSY);
		jz_write(jzfb, SLCD_DATA_RS_COMMAND | ((cmd&0xff00) << 2) | ((cmd&0xff) << 1), JZ_REG_SLCD_DATA);
		break;
	default:
		break;
	}
...
static void send_panel_data(struct jzfb *jzfb, u32 data)
{
	...
	case SLCD_CFG_DWIDTH_18:
		...
		jz_write(jzfb, SLCD_DATA_RS_DATA | ((data<<6)&0xfc0000)|((data<<4)&0xfc00) | ((data<<2)&0xfc), JZ_REG_SLCD_DATA);
		break;
	case SLCD_CFG_DWIDTH_16:
		while (jz_read(jzfb, JZ_REG_SLCD_STATE) & SLCD_STATE_BUSY);
		jz_write(jzfb, SLCD_DATA_RS_DATA | (data&0xffff), JZ_REG_SLCD_DATA);
		break;
	case SLCD_CFG_DWIDTH_9_x2:
		data = ((data & 0xff) << 1) | ((data & 0xff00) << 2);
		data = ((data << 6) & 0xfc0000) | ((data << 4) & 0xfc00) | ((data << 2) & 0xfc);
		while (jz_read(jzfb, JZ_REG_SLCD_STATE) & SLCD_STATE_BUSY);
		jz_write(jzfb, SLCD_DATA_RS_DATA | data, JZ_REG_SLCD_DATA);
		break;
	default:
		while (jz_read(jzfb, JZ_REG_SLCD_STATE) & SLCD_STATE_BUSY);
		jz_write(jzfb, SLCD_DATA_RS_DATA | (data&0xffff), JZ_REG_SLCD_DATA);
...
#define ILI9331_GPIO_CS_N 	(32 + 17)	/* Chip select */
#define ILI9331_GPIO_RESET_N 	(32 + 18)	/* LCD reset */
